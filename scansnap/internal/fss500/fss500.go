...
func Preread(dev io.ReadWriter, geometry ScanGeometry, profile ScanProfile) error {
	// request:
	// 000: 43 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 C...............
	...
	extra := prereadPayload(profile, geometry)
...
func setWindowPayload(profile ScanProfile, geometry ScanGeometry) []byte {
	switch profile {
	case ProfileTest300Geometry:
		return []byte{
			...
			byte((geometry.WidthPx - 3) >> 8), byte(geometry.WidthPx - 3), 0x00, 0x00,
			byte((geometry.HeightPx - 1) >> 8), byte(geometry.HeightPx - 1), 0x00, 0x00,
			0x00, 0x05, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
			0xc1, 0x80, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x00, 0x00, 0x00, byte((geometry.WidthPx - 64) >> 8), byte(geometry.WidthPx - 64), 0x00,
			0x00, 0x00, byte((geometry.HeightPx - 72) >> 8), byte(geometry.HeightPx - 72), 0x00, 0x00,
	...
	case ProfileStable300, ProfileTest300ResolutionOnly:
		return []byte{
			...
			0x26, 0xc3, 0x00, 0x00, 0x36, 0xd1, 0x00, 0x00, 0x80, 0x00, byte(geometry.Resolution >> 8), byte(geometry.Resolution), byte(geometry.Resolution >> 8), byte(geometry.Resolution), 0x00, 0x00,
			0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x26, 0xc0, 0x00, 0x00, 0x36, 0xd0, 0x00, 0x00,
			0x00, 0x05, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
func SetWindow(dev io.ReadWriter, geometry ScanGeometry, profile ScanProfile) error {
	// request:
	// 000: 43 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 C...............
	...
	extra := setWindowPayload(profile, geometry)
